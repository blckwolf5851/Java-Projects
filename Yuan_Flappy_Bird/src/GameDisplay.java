...
class GameDisplay extends JPanel implements ActionListener{	
	Var var;
	GameDisplay(Var var){
		this.var = var;
	}
	...
	public void paintComponent(Graphics g) {
		...
		this.setSize(var.screenSize, var.screenSize);
		g.setColor(Color.WHITE);
		g.fillOval(var.x, var.y, var.r * 2, var.r * 2);
		for (int i = 0; i < var.pipX.size(); i++) {

			g.fillRect(var.pipX.get(i), 0, var.width, var.pipH.get(i));
			g.fillRect(var.pipX.get(i), var.pipS.get(i) + var.pipH.get(i), var.width, var.screenSize - (var.pipS.get(i) + var.pipH.get(i)));
